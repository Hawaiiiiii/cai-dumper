...
## Branch model
- `main`: production-ready history. Tags for releases land here. Keep `master` only if it is the current default; otherwise prefer `main`.
...
4. Keep PRs scoped and reference issues. Target `develop` unless you are finishing a release/hotfix into `main`.
...
- Run `npm ci` then `npm run build` (set `$env:GH_TOKEN="<token>"` in PowerShell if needed for `electron-builder`) before requesting review.
